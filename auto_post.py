...
def read_message_from_file(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if not content:
                return f"Auto message at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            return content
    except Exception as e:
        print(f"❌ 读取文件错误: {e}")
        return f"Error reading file - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"

def post_to_scribbled(content, color="#FFFF88", recipient="ai", to="ai"):
    """发送消息到 scribbledthought.com"""
